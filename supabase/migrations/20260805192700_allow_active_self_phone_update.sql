...
begin
  if auth.uid() = old.id then
    ...
    if old.account_status <> 'pending' and (
      ...
      raise exception 'identity data can only be changed by the account owner while pending';
    end if;

    if old.account_status not in ('pending', 'active')
       and new.phone is distinct from old.phone then
      raise exception 'phone can only be changed by the account owner while pending or active';
    end if;
  end if;
